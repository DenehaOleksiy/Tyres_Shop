...
public class UserValidator implements Validator{
    ...
    public void validate(Object o, Errors errors) {
        ...
        ValidationUtils.rejectIfEmptyOrWhitespace(errors,"emailAdress","email.empty");
        ValidationUtils.rejectIfEmptyOrWhitespace(errors,"phoneNumber","phone.empty");
        ValidationUtils.rejectIfEmptyOrWhitespace(errors,"password","password.empty");

        if(!user.getPassword().equals(user.getPasswordConfirm())){
            errors.rejectValue("password", "password.confirm.false");
        }

        Matcher matcher;
        if(!(matcher = pattern.matcher(user.getEmailAdress())).matches()){
            errors.rejectValue("emailAdress", "bad.email");
        }
